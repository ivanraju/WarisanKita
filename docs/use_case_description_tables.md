...
| :---: | :---: |
| **USER** | **SYSTEM** |
| | 1. Use case begins when system prompt user to enter login credentials (on Web: Admin Username/ID; on Mobile: Username or Email) [M1: Enter credentials] [FR001_1] |
...
| **CONSTRAINTS** | |
| C1 : Password length $\ge$ 8 characters<br>C2 : Unapproved Artisan roles (status == 'PENDING_APPROVAL' or 'PENDING') MUST route to the Application Status Screen when Master Artisan mode is selected<br>C3 : User status == 'SUSPENDED' MUST strictly block authentication<br>C4 : Accounts with Artisan privileges MUST be offered session mode selection (Artisan Studio vs Tourist Explorer) upon login<br>C5 : Administrator moderation tools MUST be hosted on Desktop Web Portal (`https://warisan-kita.vercel.app`); Mobile app strictly guards against admin login by providing direct browser link | |
| **FUNCTIONAL REQUIREMENTS** | |
| **FR001_1** | System prompt user to enter login credentials (supporting Admin Username on Web and username/email on mobile) with multilingual UI support |
...
| **FR001_5** | System enforce platform separation by redirecting administrator accounts on mobile devices to the live Web Portal |
...
| 3. User enter registered email address and submit [A5: Administrator reset restriction] | |
...
| **FR003_5** | System enforce security restriction blocking self-service password resets for Administrator credentials |
...
| | 1. Use case begins when system retrieve and display pending artisan profile applications dynamically synchronized from Cloud DB [FR100_1] |
