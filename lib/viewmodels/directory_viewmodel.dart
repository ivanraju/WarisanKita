...
class DirectoryViewModel extends ChangeNotifier {
  ...
  Timer? _workshopRefreshDebounce;
  ...
  Future<void> fetchArtisans() async {
    ...
    try {
      ...
      if (_refreshQueued) {
        ...
        scheduleMicrotask(fetchArtisans);
      }
    }
  }

  void _watchWorkshopUpdates() {
    _workshopSubscription = _repository.watchWorkshopLocations().listen(
      (_) {
        _workshopRefreshDebounce?.cancel();
        _workshopRefreshDebounce = Timer(
          const Duration(milliseconds: 400),
          fetchArtisans,
        );
      },
      onError: (Object error, StackTrace stackTrace) {
        debugPrint('Directory workshop realtime error: $error');
  ...
  void _applyFilters() {
    ...
    final craftTokens = craftQ
        ...
        .where(
          (t) =>
              t.length >= 3 && !{'and', 'the', 'crafts', 'craft'}.contains(t),
        )
        .toList();
    ...
    _filteredArtisans = _allArtisans.where((artisan) {
      ...
      final matchesCraft =
          ...
          (craftTokens.isNotEmpty &&
              craftTokens.any(
                (t) => craft.contains(t) || tags.any((tag) => tag.contains(t)),
              ));
